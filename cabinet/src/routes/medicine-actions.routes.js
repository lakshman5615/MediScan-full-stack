// Medicine Actions - Taken/Missed Routes
// src/routes/medicine-actions.routes.js
...
// Medicine Taken - Reduce quantity by 1
router.post('/taken/:medicineId', authMiddleware, async (req, res) => {
  try {
    ...
    // Check for low stock after taking
...
// Medicine Missed - Keep quantity same
router.post('/missed/:medicineId', authMiddleware, async (req, res) => {
  try {
    ...
    // Quantity remains same (no change)
    // Just log the missed dose
    ...
    res.json({
      ...
      medicine: {
        name: medicine.medicineName,
        quantity: medicine.quantity, // Same quantity
